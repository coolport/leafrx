  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView style={styles.screen} showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 100 }}>
        <LinearGradient
          colors={colors.headerGradient as any} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }}
        ...
        <View style={[styles.section, { marginTop: -20 }]}>
              ...
              {PLANT_FILTERS.map((f) => {
                ...
                return (
                  <TouchableOpacity
                    ...
                    activeOpacity={0.7}
                    style={[
                      { paddingHorizontal: 16, paddingVertical: 8, borderRadius: 12, backgroundColor: active ? color : colors.background, borderWidth: 1, borderColor: colors.border },
          ...
          {/* Result count */}
          <Text style={{ fontSize: 11, fontWeight: "800", color: colors.textMuted, letterSpacing: 1, textTransform: "uppercase", marginBottom: 14 }}>
              ...
              <View key={disease.id}>
                <TouchableOpacity activeOpacity={0.85} onPress={() => openDetail(disease)}
                  ...
                  <View style={{ padding: 12 }}>
                    ...
                    <Text style={{ fontSize: 11, color: colors.textSecondary, fontWeight: "500", lineHeight: 15 }} numberOfLines={2}>{disease.tagline}</Text>
      ...
      <Modal visible={!!selected} animationType="slide" transparent onRequestClose={() => setSelected(null)}>
        {selected && (
          <View style={{ flex: 1, justifyContent: "flex-end", backgroundColor: colors.modalBackground }}>
            <View style={{ backgroundColor: colors.card, borderTopLeftRadius: 28, borderTopRightRadius: 28, height: SH - insets.top - 20, overflow: "hidden" }}>
              ...
              {/* Tabs */}
              <View style={{ flexDirection: "row", backgroundColor: colors.background, borderBottomWidth: 1, borderBottomColor: colors.border, flexShrink: 0 }}>
                {(["overview", "symptoms", "treatment", "prevention"] as const).map((tab) => (
                  <TouchableOpacity key={tab} onPress={() => setDetailTab(tab)}
                    ...
                    <Text style={{ fontSize: 11, fontWeight: "800", color: detailTab === tab ? getPlantColor(selected.plant) : colors.textMuted, textTransform: "uppercase", letterSpacing: 0.5 }}>
              ...
              <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ padding: 20, paddingBottom: insets.bottom + 32 }}>
                  ...
                  <View style={{ gap: 16 }}>
                    <View style={{ flexDirection: "row", gap: 10 }}>
                      {[
                        ...
                        <View key={stat.label} style={{ flex: 1, backgroundColor: colors.background, borderRadius: 14, padding: 12, borderWidth: 1, borderColor: colors.border }}>
                          <Feather name={stat.icon as any} size={14} color={stat.color} />
                          <Text style={{ fontSize: 10, fontWeight: "700", color: colors.textMuted, marginTop: 6, letterSpacing: 0.5, textTransform: "uppercase" }}>{stat.label}</Text>
                          <Text style={{ fontSize: 13, fontWeight: "800", color: colors.text, marginTop: 2 }}>{stat.value}</Text>
                        </View>
                      ))}
                    </View>
                    {selected.scientific_name && (
                      <InfoCard label="Scientific Name" color={getPlantColor(selected.plant)} icon="book-open" colors={colors}>
                        <Text style={{ fontSize: 14, fontStyle: "italic", color: colors.textSecondary, fontWeight: "500" }}>{selected.scientific_name}</Text>
                      </InfoCard>
                    )}
                    <InfoCard label="Overview" color={getPlantColor(selected.plant)} icon="info" colors={colors}>
                      <Text style={{ fontSize: 14, color: colors.textSecondary, lineHeight: 22 }}>{selected.overview}</Text>
                    </InfoCard>
                    <InfoCard label="Pathogen Detail" color={getPlantColor(selected.plant)} icon="cpu" colors={colors}>
                      <Text style={{ fontSize: 14, color: colors.textSecondary, lineHeight: 22 }}>{selected.pathogen_detail}</Text>
                    </InfoCard>
                    {selected.affected_parts.length > 0 && (
                      <InfoCard label="Affected Parts" color={getPlantColor(selected.plant)} icon="target" colors={colors}>
                    ...
                    <InfoCard label="Conditions Favoring Disease" color={getPlantColor(selected.plant)} icon="cloud-rain" colors={colors}>
                      {[
                        ...
                        <View key={i} style={{ flexDirection: "row", marginBottom: 8, gap: 8 }}>
                          <Text style={{ fontSize: 12, fontWeight: "800", color: colors.textMuted, width: 90 }}>{row.k}:</Text>
                          <Text style={{ flex: 1, fontSize: 13, color: colors.textSecondary, lineHeight: 18 }}>{row.v}</Text>
                        </View>
                      ))}
                    </InfoCard>
                    <InfoCard label="Economic Impact" color={colors.danger} icon="trending-down" colors={colors}>
                      <Text style={{ fontSize: 14, color: colors.textSecondary, lineHeight: 22 }}>{selected.economic_impact}</Text>
                    </InfoCard>
                    {selected.look_alikes.length > 0 && (
                      <InfoCard label="Could Be Confused With" color={colors.warning} icon="eye" colors={colors}>
                        {selected.look_alikes.map((item, i) => <BulletRow key={i} text={item} color={colors.warning} colors={colors} />)}
                      </InfoCard>
                    )}
                    <InfoCard label="Recovery Timeline" color={getPlantColor(selected.plant)} icon="clock" colors={colors}>
                      <Text style={{ fontSize: 14, color: colors.textSecondary, lineHeight: 22 }}>{selected.recovery_timeline}</Text>
                    </InfoCard>
                    <InfoCard label="Did You Know?" color="#8b5cf6" icon="star" colors={colors}>
                      <Text style={{ fontSize: 14, color: colors.textSecondary, lineHeight: 22, fontStyle: "italic" }}>{selected.fun_fact}</Text>
                    </InfoCard>
                  </View>
                )}
                {detailTab === "symptoms" && (
                  <View style={{ gap: 16 }}>
                    <InfoCard label="Early Warning Signs" color={colors.warning} icon="alert-circle" colors={colors}>
                      {selected.symptoms.early.map((s, i) => <BulletRow key={i} text={s} color={colors.warning} colors={colors} />)}
                    </InfoCard>
                    <InfoCard label="Advanced / Severe Symptoms" color={colors.danger} icon="alert-triangle" colors={colors}>
                      {selected.symptoms.advanced.map((s, i) => <BulletRow key={i} text={s} color={colors.danger} colors={colors} />)}
                    </InfoCard>
                    <InfoCard label="How to Distinguish from Look-alikes" color={getPlantColor(selected.plant)} icon="crosshair" colors={colors}>
                      <Text style={{ fontSize: 14, color: colors.textSecondary, lineHeight: 22 }}>{selected.symptoms.distinguishing}</Text>
                    </InfoCard>
                  </View>
                )}
                {detailTab === "treatment" && (
                  <View style={{ gap: 16 }}>
                    <InfoCard label="Immediate Actions" color={colors.danger} icon="zap" colors={colors}>
                      {selected.treatment.immediate.map((a, i) => <BulletRow key={i} text={a} color={colors.danger} colors={colors} icon="chevron-right" />)}
                    </InfoCard>
                    {selected.treatment.chemical.length > 0 && (
                      <InfoCard label="Chemical Controls" color={colors.secondary} icon="droplet" colors={colors}>
                        {selected.treatment.chemical.map((c, i) => (
                          <View key={i} style={{ backgroundColor: colors.background, borderRadius: 14, padding: 12, marginBottom: 10, borderLeftWidth: 3, borderLeftColor: colors.secondary }}>
                            <Text style={{ fontSize: 13, fontWeight: "800", color: colors.text, marginBottom: 4 }}>{c.product}</Text>
                            <View style={{ backgroundColor: `${colors.secondary}33`, paddingHorizontal: 8, paddingVertical: 3, borderRadius: 8, alignSelf: "flex-start" }}>
                              <Text style={{ fontSize: 11, fontWeight: "700", color: colors.secondary }}>Rate: {c.rate}</Text>
                            </View>
                            <Text style={{ fontSize: 12, color: colors.textSecondary, marginTop: 6, lineHeight: 17 }}>⏱ {c.frequency}</Text>
                          </View>
                        ))}
                      </InfoCard>
                    )}
                    {selected.treatment.organic.length > 0 && (
                      <InfoCard label="Organic / Biological Options" color={colors.success} icon="wind" colors={colors}>
                        {selected.treatment.organic.map((o, i) => <BulletRow key={i} text={o} color={colors.success} colors={colors} />)}
                      </InfoCard>
                    )}
                    {selected.treatment.cultural.length > 0 && (
                      <InfoCard label="Cultural Practices" color="#8b5cf6" icon="tool" colors={colors}>
                        {selected.treatment.cultural.map((c, i) => <BulletRow key={i} text={c} color="#8b5cf6" colors={colors} />)}
                      </InfoCard>
                    )}
                  </View>
                )}
                {detailTab === "prevention" && (
                  <View style={{ gap: 16 }}>
                    <InfoCard label="Prevention Strategies" color={getPlantColor(selected.plant)} icon="shield" colors={colors}>
                      {selected.prevention.map((p, i) => <BulletRow key={i} text={p} color={getPlantColor(selected.plant)} colors={colors} />)}
                    </InfoCard>
                    <InfoCard label="Monitoring Tips" color={colors.secondary} icon="activity" colors={colors}>
                      {selected.monitoring_tips.map((m, i) => <BulletRow key={i} text={m} color={colors.secondary} colors={colors} />)}
                    </InfoCard>
                  </View>
                )}
              </ScrollView>
...
function InfoCard({ label, color, icon, children, colors }: { label: string; color: string; icon: string; children: React.ReactNode; colors: any }) {
  return (
    <View style={{ backgroundColor: colors.card, borderRadius: 18, padding: 16, borderWidth: 1, borderColor: colors.border, elevation: 2 }}>
      <View style={{ flexDirection: "row", alignItems: "center", gap: 7, marginBottom: 12 }}>
        <View style={{ backgroundColor: color + "18", padding: 6, borderRadius: 9 }}>
          <Feather name={icon as any} size={13} color={color} />
        </View>
        <Text style={{ fontSize: 11, fontWeight: "800", color: colors.textMuted, letterSpacing: 0.8, textTransform: "uppercase" }}>{label}</Text>
      </View>
      {children}
    </View>
  );
...
function BulletRow({ text, color, colors, icon = "check" }: { text: string; color: string; colors: any; icon?: string }) {
  return (
    <View style={{ flexDirection: "row", alignItems: "flex-start", gap: 10, marginBottom: 8 }}>
      ...
      <Text style={{ flex: 1, fontSize: 13, color: colors.textSecondary, lineHeight: 19, fontWeight: "500" }}>{text}</Text>
    </View>
  );
}
